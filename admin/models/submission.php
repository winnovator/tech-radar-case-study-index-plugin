...
class Submission {
    ...
    public function checkForDuplicate($formID, $seqID) {
        $this->setDbObj();
        $preparedStmt = $this->dbObj->prepare("SELECT COUNT(seq_id) FROM {$this->dbObj->prefix}qt_published WHERE form_id = %d AND seq_id = %d", [(int)$formID, (int)$seqID]);
        return $this->dbObj->get_var($preparedStmt);
    }

    public function addSubmissionToPublished($formID, $seqID) {
        $this->setDbObj();
        $this->dbObj->insert("{$this->dbObj->prefix}qt_published", ['form_id' => (int)$formID, 'seq_id' => (int)$seqID]);
    }

    public function deleteSubmissionFromPublished($formID, $seqID) {
        $this->setDbObj();
        $this->dbObj->delete("{$this->dbObj->prefix}qt_published", ['form_id' => (int)$formID, 'seq_id' => (int)$seqID]);
    }

    public function deleteAllPublishedSubmissionByFormID($formID) {
        $this->setDbObj();
        $this->dbObj->delete("{$this->dbObj->prefix}qt_published", ['form_id' => (int)$formID]);
    }
